...
% #A modified webdriver_stocha for random webs. 
% #The results will be saved to QNAP hard drive
% #Here the number of random webs analyzed is fixed 
% #Run this after you have ran FilterOutput.m 
% #mijuvest 30/07/2018


load('filteredwebs.mat', 'filtered_outputStructure');
path = '\\192.168.21.132\home';

% going thru first 30 webs (out of 113 available)
for webindex = 1:30   

     
    Data = filtered_outputStructure(webindex).data; 
    pathTemp1 = strcat('\testresults\',datestr(now,'YYYY-mm-DD_HH-MM-ss'));
    pathTemp = strcat(pathTemp1, '\');
    pathFinal = strcat(path, pathTemp);

    
    %ResultOptions.Directory = sprintf('./testresults/%s/',datestr(now,'YYYY-mm-DD_HH-MM-ss'));
    ResultOptions.Directory = pathFinal; 
