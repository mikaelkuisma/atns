...
function [nicheweb_new,Mass,orig_nodes,species,N_stages,is_split,aging_table,fecund_table,n,clumped_web,Anna_stuff]= LifeHistories_ATN(lifehis,leslie,orig,nichewebsize,connectance)
...
W_inf=W_max/infsize;
% Next, we find t_0, which is the x-intercept of a weight vs. age plot.q
...
for i=find(is_split')
    ...
    % the following added by mijuvest 16.5.2019 to ensure that Mass vector
    % has a right length! Otherwise "isnan" below removes NaN-entries
    % (there are NaNs when no age classes for fishes. 
    if t_max(i) == 0
        Mass_matrix(i,t+1)=Mvec(i);
    end
    %%%%%%%%
...
%%-------------------------------------------------------------------------
[aging_table,fecund_table]= LeslieMatrix_ATN(leslie,newwebsize,N_stages,is_split,species);
...
if (fishpred==2 || splitdiet==false)
    ...
    [web_mx]=CreateWeb_ATN(sum(N_stages),connectance,n,n_new,r_new,c_new,orig_index,givediet);
else
