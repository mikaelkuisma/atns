...
%--------------------------------------------------------------------------
% Modified by mvesteri Feb 21 2018 for ATN purposes
% Input parameters: 
% cannibal_invert: Invertebrate Cannibalism
% num_species: number of species
% connectance = the relative portion of connections
% Output: the web structure, added basal species

function [orig]= NicheModel_ATN(cannibal_invert,num_species, connectance)
    ...
    while (tries>0 && ok_n==0)
        ...
        [nicheweb,orig.n_new,orig.r_new,orig.c_new,c]=CreateWeb_ATN(num_species,connectance,n);
        ...
        %% 5. Discard webs if there occurs "cyclic behaviour for eating": (i eats j that eats i)
        % mvesterinen 19.6.2018
        
        if CyclicEating(nicheweb)
            ok_n = 0; 
        end
    ...
    orig.basalsp = basalsp;         % saving the basal species to the struct
    orig.c = c; 
end
